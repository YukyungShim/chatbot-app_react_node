...
function Chatbot() {
    ...
    // OpenAI API와의 통신을 위한 textQuery 수정
    const textQuery = async (text) => {
        ...
        try {
            // OpenAI API로 요청 보내기
            const response = await Axios.post('/api/openai/textQuery', textQueryVariables);

            for (let content of response.data.choices) {
                conversation = {
                    who: 'bot',
                    content: {
                        text: {
            ...
            conversation = {
                ...
                content: {
                    text: {
                        text: "An error occurred, please check the problem.",
                    },
                },
            };
            dispatch(saveMessage(conversation));
    ...
    // OpenAI API로 이벤트 쿼리 보내기
    const eventQuery = async (event) => {
        const eventQueryVariables = { event };
        try {
            // OpenAI API로 요청 보내기
            const response = await Axios.post('/api/openai/eventQuery', eventQueryVariables);
